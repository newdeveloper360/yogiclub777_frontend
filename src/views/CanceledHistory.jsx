    ...
    <>
      ...
      <div className="w-full  overflow-auto">
        <table className="w-full text-xs table-auto">
          ...
          <tbody>
            {!dataLoading && historyData.filter(dataItem => dataItem.status === 'CANCELED').map((dataItem, dataItemIdx) => (
              <tr id={`Row${dataItem.gameId}`} className="text-center">
                <td className="p-1">{dataItemIdx + 1 + (currentPage - 1) * perPageRecords}</td>
                <td className="p-1">{dataItem["updated_at"]}</td>
                ...
                <td className="p-1">{dataItem["amount"]}<br /><small>(Refunded)</small></td>
